...
$customTile = $this->_options_4['bdt_tile_layer'] ?? '';
$setTileLayer = (is_string($customTile) && strpos($customTile, '{') !== false)
    ? $customTile
...
<style>
    .leaflet-container a {
        color: <?= esc_attr($options['primary_color'] ?? '#007bff'); ?> !important;
    }
</style>
    ...
    document.addEventListener("DOMContentLoaded", function() {
            ...
            <?php foreach ($companiesFeed->companies as $company) : ?>
                <?php if (!empty($company->coordinates->latitude)) : ?>
                    L.marker([<?= json_encode($company->coordinates->latitude); ?>, <?= json_encode($company->coordinates->longitude); ?>], {icon: coloredIcon}).addTo(map)
                        .bindPopup("<?= '<b>' . esc_js($company->name) . '</b><br>' . esc_js($company->address) . '<br>' . esc_js($company->postNumber) . ' ' . esc_js($company->city) . '<br>' . ($company->phone != 0 ? '<a href=\"tel:+45' . esc_js($company->phone) . '\">+45 ' . esc_js($company->phone) . '</a><br>' : '') . (!empty($options_6['departments_google_directions_' . $company->id]) ? '<a href=\"' . esc_url($options_6['departments_google_directions_' . $company->id]) . '\" target=\"_blank\">Rutevejledning</a>' : '') ?>");
                <?php endif; ?>
            <?php endforeach; ?>
        <?php endif; ?>
        ...
        <?php if(!empty($atts['detailspage']) && $atts['detailspage'] == 'true' && !empty($this->currentVehicle->company->coordinates->latitude)) : ?>
            L.marker([<?= json_encode($this->currentVehicle->company->coordinates->latitude); ?>, <?= json_encode($this->currentVehicle->company->coordinates->longitude); ?>], {icon: coloredIcon}).addTo(map)
                .bindPopup("<?= '<b>' . esc_js($this->currentVehicle->company->name) . '</b><br>' . esc_js($this->currentVehicle->company->address) . '<br>' . esc_js($this->currentVehicle->company->postNumber) . ' ' . esc_js($this->currentVehicle->company->city) . '<br>' . ($this->currentVehicle->company->phone != 0 ? '<a href=\"tel:+45' . esc_js($this->currentVehicle->company->phone) . '\">+45 ' . esc_js($this->currentVehicle->company->phone) . '</a><br>' : '') . (!empty($options_6['departments_google_directions_' . $this->currentVehicle->company->id]) ? '<a href=\"' . esc_url($options_6['departments_google_directions_' . $this->currentVehicle->company->id]) . '\" target=\"_blank\">Rutevejledning</a>' : '') ?>");
        <?php endif; ?>
    });
</script>